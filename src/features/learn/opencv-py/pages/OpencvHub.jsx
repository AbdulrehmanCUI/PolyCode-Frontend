...
const LEARNING_PATH = [
  {
    level: "Beginner",
    chapters: ["intro", "pixels", "drawing"],
    color: "#27ae60",
    summary: "Install OpenCV, read images, pixels, drawing, and ROI.",
  },
  {
    level: "Intermediate",
    chapters: ["colors", "arithmetic", "filtering", "edges"],
    color: "#2ecc71",
    summary: "Color spaces, blending, filters, morphology, and edges.",
  },
  {
    level: "Advanced",
    chapters: ["contours", "geometry", "video"],
    color: "#f39c12",
    summary: "Contours, warps, features, video, and detection.",
  },
  {
    level: "Capstone",
    chapters: ["capstone"],
    color: "#5CBF2A",
    summary: "Mini pipelines, tracker sketch, and cheat sheet.",
  },
];
...
export default function OpencvHub() {
  ...
  const earnedXP = OPENCV_LESSONS.filter((lesson) => progress[lesson.id]).reduce(
    (sum, lesson) => sum + lesson.xp,
    0,
  );
  const pct = Math.round((completedCount / OPENCV_LESSONS.length) * 100) || 0;
  const nextLesson =
    OPENCV_LESSONS.find((lesson) => !progress[lesson.id]) || OPENCV_LESSONS[0];
  const resumeLesson =
    OPENCV_LESSONS.find((lesson) => lesson.id === lastLessonId) || nextLesson;
  const completedChapters = OPENCV_CHAPTERS.filter((chapter) =>
    chapter.lessons.every((lesson) => progress[lesson.id]),
  ).length;
  const bookmarkedLessons = bookmarks
    .map((id) => OPENCV_LESSONS.find((lesson) => lesson.id === id))
    .filter(Boolean);

  const filteredLessons = useMemo(() => {
    const query = search.trim().toLowerCase();
    return OPENCV_LESSONS.filter((lesson) => {
      const matchesQuery =
        !query ||
  ...
  return (
    <div className="oops-hub opencv-hub">
      <div className="oops-hero opencv-hero">
        <Link
          to="/language/Python"
        ...
        <div className="oops-hero-badge">OPENCV · PYTHON TRACK</div>
        ...
        <p className="oops-hero-sub">
          Computer vision with Python — images, filters, edges, contours,
          features, video, detection, and a hands-on capstone.
        </p>
        ...
        <div className="oops-hero-grid">
          <div className="oops-xp-bar-wrap">
            <div className="oops-xp-meta">
              <span>
                {isAuthenticated
                  ? `${completedCount}/${OPENCV_LESSONS.length} lessons · ${earnedXP}/${OPENCV_TOTAL_XP} XP`
                  : `Sign in to track progress · ${OPENCV_LESSONS.length} lessons`}
              </span>
          ...
          <div className="oops-resume-panel">
            ...
            >
              {completedCount > 0 ? "Resume OpenCV" : "Start OpenCV"}
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="oops-guide-tools">
        <div className="oops-tool-panel oops-tool-panel-main">
          <span className="oops-interactive-label">Find an OpenCV topic</span>
          <div className="oops-search-row">
            <input
              type="search"
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              placeholder="Search imread, Canny, contours, VideoCapture..."
              aria-label="Search OpenCV lessons"
            />
            <div
              className="oops-filter-tabs"
              aria-label="Filter OpenCV lessons"
            >
              {[
                ["all", "All"],
                ["todo", "To do"],
      ...
      <div className="oops-dashboard-strip">
        <div className="oops-stat-tile">
          <span>Lessons</span>
          <strong>
            {completedCount}/{OPENCV_LESSONS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Chapters</span>
          <strong>
            {completedChapters}/{OPENCV_CHAPTERS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>XP</span>
          <strong>
            {earnedXP}/{OPENCV_TOTAL_XP}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Bookmarks</span>
      ...
      <section className="matplotlib-learn-path" aria-label="Learning path">
        <div className="matplotlib-path-label">
          ...
          <small>
            {OPENCV_CHAPTERS.length} chapters · {OPENCV_LESSONS.length}{" "}
            lessons
          </small>
        </div>
        <div className="matplotlib-path-grid">
          {LEARNING_PATH.map((stage) => {
            const stageChapters = OPENCV_CHAPTERS.filter((ch) =>
