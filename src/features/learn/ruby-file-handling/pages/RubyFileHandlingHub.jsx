...
const LEARNING_PATH = [
  {
    level: "Beginner",
    chapters: RUBY_FILE_HANDLING_CHAPTERS.filter((ch) => ch.stage === "beginner").map((ch) => ch.id),
    color: "#10B981",
    summary: "Learn the basics of file handling: reading, writing, and understanding file objects in Ruby.",
  },
  {
    level: "Intermediate",
    chapters: RUBY_FILE_HANDLING_CHAPTERS.filter((ch) => ch.stage === "intermediate").map((ch) => ch.id),
    color: "#F59E0B",
    summary: "Work with CSV, JSON, and YAML files. Handle errors and manage file paths safely.",
  },
  {
    level: "Advanced",
    chapters: RUBY_FILE_HANDLING_CHAPTERS.filter((ch) => ch.stage === "advanced").map((ch) => ch.id),
    color: "#EF4444",
...
export default function RubyFileHandlingHub() {
  const navigate = useNavigate();
  const [search, setSearch] = useState("");
  const [stage, setStage] = useState("beginner");
  const [filter, setFilter] = useState("all");
  const { isAuthenticated, completedMap: progress, bookmarks, lastLessonId } = useRubyFileHandlingProgress();

  const completedCount = Object.keys(progress).length;
  const earnedXP = RUBY_FILE_HANDLING_LESSONS.filter((lesson) => progress[lesson.id]).reduce(
    (sum, lesson) => sum + lesson.xp,
    0,
  );
  const pct = Math.round((completedCount / RUBY_FILE_HANDLING_LESSONS.length) * 100) || 0;
  const nextLesson = RUBY_FILE_HANDLING_LESSONS.find((lesson) => !progress[lesson.id]) || RUBY_FILE_HANDLING_LESSONS[0];
  const resumeLesson = RUBY_FILE_HANDLING_LESSONS.find((lesson) => lesson.id === lastLessonId) || nextLesson;
  const chaptersForStage = RUBY_FILE_HANDLING_CHAPTERS.filter((c) => (c.stage || "beginner") === stage);
  ...
  const bookmarkedLessons = bookmarks
    .map((id) => RUBY_FILE_HANDLING_LESSONS.find((lesson) => lesson.id === id))
    .filter(Boolean);

  const filteredLessons = useMemo(() => {
    const query = search.trim().toLowerCase();
    return RUBY_FILE_HANDLING_LESSONS.filter((lesson) => {
      // only match lessons in the selected stage
      const ch = RUBY_FILE_HANDLING_CHAPTERS.find((c) => c.lessons.some((l) => l.id === lesson.id));
  ...
  return (
    <div className="oops-hub ruby-file-handling-hub">
      <div className="oops-hero ruby-file-handling-hero">
        <Link to="/language/Ruby" className="oops-back-btn" style={{ marginBottom: "0.75rem", display: "inline-flex" }}>
          ← Ruby courses
        </Link>
        <div className="oops-hero-badge">Ruby · FILE HANDLING TRACK</div>
        <h1 className="oops-hero-title">
          Ruby
          <br />
          <span className="oops-hero-accent">File Handling</span>
        </h1>
        <p className="oops-hero-sub">
          A complete Ruby file handling path from reading and writing text files to working with CSV,
          JSON, YAML, and binary files. Hands-on examples and runnable challenges.
        </p>
        ...
        <div className="oops-hero-grid">
          <div className="oops-xp-bar-wrap">
            <div className="oops-xp-meta">
              <span>
                {isAuthenticated
                  ? `${completedCount}/${RUBY_FILE_HANDLING_LESSONS.length} lessons · ${earnedXP}/${RUBY_FILE_HANDLING_TOTAL_XP} XP`
                  : `Sign in to track progress · ${RUBY_FILE_HANDLING_LESSONS.length} lessons`}
              </span>
          ...
          <div className="oops-resume-panel">
            ...
            <p>{RUBY_FILE_HANDLING_CHAPTERS.find((c) => c.lessons.some((l) => l.id === resumeLesson.id))?.title || ""} · {resumeLesson.xp} XP</p>
      ...
      <div className="oops-guide-tools">
        <div className="oops-tool-panel oops-tool-panel-main">
          <span className="oops-interactive-label">Find a file handling topic</span>
          <div className="oops-search-row">
            <input type="search" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search file reading, writing, CSV, JSON..." aria-label="Search Ruby File Handling lessons" />
            <div className="oops-filter-tabs" aria-label="Filter Ruby File Handling lessons">
          ...
          <div className="oops-search-results">
            {filteredLessons.slice(0, 6).map((lesson) => (
              <button key={lesson.id} type="button" className="oops-search-result" onClick={() => navigate(`${BASE_PATH}/lesson/${lesson.id}`)}>
                ...
                <small>{RUBY_FILE_HANDLING_CHAPTERS.find((c) => c.lessons.some((l) => l.id === lesson.id))?.title || ""}</small>
              </button>
            ))}
        ...
        <div className="oops-tool-panel">
          ...
          <p>Next in {RUBY_FILE_HANDLING_CHAPTERS.find((c) => c.lessons.some((l) => l.id === nextLesson.id))?.title || ""}. Earn {nextLesson.xp} XP.</p>
      ...
      <div className="oops-dashboard-strip">
        <div className="oops-stat-tile"><span>Lessons</span><strong>{completedCount}/{RUBY_FILE_HANDLING_LESSONS.length}</strong></div>
        <div className="oops-stat-tile"><span>Chapters</span><strong>{completedChapters}/{RUBY_FILE_HANDLING_CHAPTERS.length}</strong></div>
        <div className="oops-stat-tile"><span>XP</span><strong>{earnedXP}/{RUBY_FILE_HANDLING_TOTAL_XP}</strong></div>
      ...
      <section className="matplotlib-learn-path" aria-label="Learning path">
        ...
        <div className="matplotlib-path-grid">
          {LEARNING_PATH.map((stage) => {
            const stageChapters = RUBY_FILE_HANDLING_CHAPTERS.filter((ch) =>
      ...
      <LearnChapterPathOverview
        chapters={RUBY_FILE_HANDLING_CHAPTERS}
        progress={progress}
      ...
      {completedCount === RUBY_FILE_HANDLING_LESSONS.length && RUBY_FILE_HANDLING_LESSONS.length > 0 && (
        <CourseCertificate
          courseName="Ruby File Handling"
          totalLessons={RUBY_FILE_HANDLING_LESSONS.length}
          completedCount={completedCount}
          earnedXP={earnedXP}
          totalXP={RUBY_FILE_HANDLING_TOTAL_XP}
        />
      )}
    </div>
  );
