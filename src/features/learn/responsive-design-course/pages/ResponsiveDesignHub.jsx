...
const LEARNING_PATH = [
  {
    level: "Beginner",
    chapters: ["rwd-foundations"],
    color: "#e91e8c",
    summary: "Why RWD matters, the viewport meta tag, and fluid widths.",
  },
  {
    level: "Intermediate",
    chapters: ["rwd-media-queries", "rwd-responsive-units"],
    color: "#f59e0b",
    summary: "Media query syntax, mobile-first design, and em/rem/viewport units plus clamp().",
  },
  {
    level: "Advanced",
    chapters: ["rwd-images-typography", "rwd-layout-patterns"],
    color: "#7c3aed",
    summary: "Responsive images, fluid typography, hamburger menus, and container queries.",
  },
  {
    level: "Expert",
    chapters: ["rwd-testing-capstone"],
    color: "#dc2626",
    summary: "Testing responsiveness, common pitfalls, and a capstone responsive landing page.",
  },
];
...
export default function ResponsiveDesignHub() {
  ...
  const pct =
    Math.round((completedCount / RESPONSIVE_DESIGN_LESSONS.length) * 100) || 0;

  const nextLesson =
    RESPONSIVE_DESIGN_LESSONS.find((lesson) => !progress[lesson.id]) ||
    RESPONSIVE_DESIGN_LESSONS[0];
  const resumeLesson =
    RESPONSIVE_DESIGN_LESSONS.find((lesson) => lesson.id === lastLessonId) ||
    nextLesson;
  const completedChapters = RESPONSIVE_DESIGN_CHAPTERS.filter((chapter) =>
    chapter.lessons.every((lesson) => progress[lesson.id]),
  ).length;
  const bookmarkedLessons = bookmarks
    .map((id) => RESPONSIVE_DESIGN_LESSONS.find((lesson) => lesson.id === id))
    .filter(Boolean);

  const filteredLessons = useMemo(() => {
    const query = search.trim().toLowerCase();
    return RESPONSIVE_DESIGN_LESSONS.filter((lesson) => {
      const matchesQuery =
        !query ||
    ...
    <div className="oops-hub matplotlib-hub">
      <div className="oops-hero matplotlib-hero">
        ...
        <p className="oops-hero-sub">
          Build pages that adapt to any screen — media queries, fluid units, responsive images, and modern layout patterns like container queries. {RESPONSIVE_DESIGN_CHAPTERS.length}{" "}
          chapters, {RESPONSIVE_DESIGN_LESSONS.length} lessons, hands-on challenges.
        </p>
        ...
        <div className="oops-hero-grid">
          <div className="oops-xp-bar-wrap">
            <div className="oops-xp-meta">
              <span>
                {isAuthenticated
                  ? `${completedCount}/${RESPONSIVE_DESIGN_LESSONS.length} lessons · ${earnedXP}/${RESPONSIVE_DESIGN_TOTAL_XP} XP`
                  : `Sign in to track progress · ${RESPONSIVE_DESIGN_LESSONS.length} lessons`}
              </span>
      ...
      <div className="oops-guide-tools">
        <div className="oops-tool-panel oops-tool-panel-main">
          <span className="oops-interactive-label">Find a Responsive Web Design topic</span>
          <div className="oops-search-row">
            <input
              type="search"
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              placeholder="Search media queries, breakpoints, units..."
              aria-label="Search Responsive Web Design lessons"
            />
            <div
              className="oops-filter-tabs"
              aria-label="Filter Responsive Web Design lessons"
            >
              {[
                ["all", "All"],
                ["todo", "To do"],
      ...
      <div className="oops-dashboard-strip">
        <div className="oops-stat-tile">
          <span>Lessons</span>
          <strong>
            {completedCount}/{RESPONSIVE_DESIGN_LESSONS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Chapters</span>
          <strong>
            {completedChapters}/{RESPONSIVE_DESIGN_CHAPTERS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>XP</span>
          <strong>
            {earnedXP}/{RESPONSIVE_DESIGN_TOTAL_XP}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Bookmarks</span>
      ...
      <section className="matplotlib-learn-path" aria-label="Learning path">
        <div className="matplotlib-path-label">
          ...
          <small>
            {RESPONSIVE_DESIGN_CHAPTERS.length} chapters ·{" "}
            {RESPONSIVE_DESIGN_LESSONS.length} lessons
          </small>
        </div>
        <div className="matplotlib-path-grid">
          {LEARNING_PATH.map((stage) => {
            const stageChapters = RESPONSIVE_DESIGN_CHAPTERS.filter((ch) =>
