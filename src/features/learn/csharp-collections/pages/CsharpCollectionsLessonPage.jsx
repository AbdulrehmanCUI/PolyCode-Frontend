...
const READ_GATE_PREFIX = "csharp_collections";

export default function CsharpCollectionsLessonPage() {
  ...
  const lesson = CSHARP_COLLECTIONS_LESSONS.find((item) => item.id === lessonId);
  const lessonIdx = CSHARP_COLLECTIONS_LESSONS.findIndex(
    (item) => item.id === lessonId,
  );
  const prev = CSHARP_COLLECTIONS_LESSONS[lessonIdx - 1];
  const next = CSHARP_COLLECTIONS_LESSONS[lessonIdx + 1];

  useLessonAssistantContext({
    course: "C# Collections",
    language: "C#",
    lesson,
  ...
  if (!lesson) {
    return (
      <div className="oops-not-found">
        <p>C# Collections lesson not found.</p>
        <button type="button" onClick={() => navigate(BASE_PATH)}>
          ← Back to C# Collections
        </button>
      </div>
    );
  }
  ...
  const earnedXP = CSHARP_COLLECTIONS_LESSONS.filter((item) => progress[item.id]).reduce(
    ...
    <div className={`oops-lesson-page ${focusMode ? "oops-focus-mode" : ""}`}>
      ...
      <div className="oops-lesson-main">
        <div className="oops-lesson-topbar">
          ...
          <LearnProfileMenu
            user={user}
            trackTitle="C# Collections"
            syncLabel={
              isAuthenticated
                ? "C# Collections progress saved to your account"
        ...
        <LessonContentShell
          tab={tab}
          storageKey={`csharp-collections:${lessonId}`}
          videoUrl={lesson.videoUrl}
          videoTitle={`${lesson.title} — C# Collections`}
        >
          {tab === "theory" ? (
            <NumpyIntroTheory
              lesson={lesson}
