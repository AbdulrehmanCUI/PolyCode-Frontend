...
import {
  CSHARP_LINQ_CHAPTERS,
  CSHARP_LINQ_LESSONS,
  CSHARP_LINQ_TOTAL_XP,
} from "../data/csharpLinqCurriculum";
import useCsharpLinqProgress from "../hooks/useCsharpLinqProgress";
import LearnChapterPathOverview from "../../shared/LearnChapterPathOverview";
import LearnChapterGrid from "../../shared/LearnChapterGrid";
import CourseCertificate from "../../shared/CourseCertificate";

const BASE_PATH = "/learn/csharp-linq";
...
export default function CsharpLinqHub() {
  ...
  const {
    ...
    bookmarks,
    lastLessonId,
  } = useCsharpLinqProgress();

  const completedCount = Object.keys(progress).length;
  const earnedXP = CSHARP_LINQ_LESSONS.filter(
    (lesson) => progress[lesson.id],
  ).reduce((sum, lesson) => sum + lesson.xp, 0);
  const pct =
    Math.round((completedCount / CSHARP_LINQ_LESSONS.length) * 100) || 0;
  const nextLesson =
    CSHARP_LINQ_LESSONS.find((lesson) => !progress[lesson.id]) ||
    CSHARP_LINQ_LESSONS[0];
  const resumeLesson =
    CSHARP_LINQ_LESSONS.find((lesson) => lesson.id === lastLessonId) ||
    nextLesson;
  const completedChapters = CSHARP_LINQ_CHAPTERS.filter((chapter) =>
    chapter.lessons.every((lesson) => progress[lesson.id]),
  ).length;
  const bookmarkedLessons = bookmarks
    .map((id) => CSHARP_LINQ_LESSONS.find((lesson) => lesson.id === id))
    .filter(Boolean);

  const filteredLessons = useMemo(() => {
    const query = search.trim().toLowerCase();
    return CSHARP_LINQ_LESSONS.filter((lesson) => {
      const matchesQuery =
        !query ||
  ...
  return (
    <div className="oops-hub csharp-linq-hub">
      <div className="oops-hero csharp-linq-hero">
        <Link
          to="/select-language"
        ...
        <div className="oops-hero-badge">C# · LINQ TRACK</div>
        <h1 className="oops-hero-title">
          C#
          <br />
          <span className="oops-hero-accent">LINQ</span>
        </h1>
        <p className="oops-hero-sub">
          Query collections with Where, Select, OrderBy, GroupBy, and
          aggregation methods — then chain them into readable pipelines.
        </p>
        ...
        <div className="oops-hero-grid">
          <div className="oops-xp-bar-wrap">
            <div className="oops-xp-meta">
              <span>
                {isAuthenticated
                  ? `${completedCount}/${CSHARP_LINQ_LESSONS.length} lessons · ${earnedXP}/${CSHARP_LINQ_TOTAL_XP} XP`
                  : `Sign in to track progress · ${CSHARP_LINQ_LESSONS.length} lessons`}
              </span>
          ...
          <div className="oops-resume-panel">
            ...
            >
              {completedCount > 0 ? "Resume C# LINQ" : "Start C# LINQ"}
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="oops-guide-tools">
        <div className="oops-tool-panel oops-tool-panel-main">
          <span className="oops-interactive-label">Find a C# LINQ topic</span>
          <div className="oops-search-row">
            <input
              type="search"
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              placeholder="Search Where, Select, GroupBy, OrderBy..."
              aria-label="Search C# LINQ lessons"
            />
            <div className="oops-filter-tabs" aria-label="Filter C# LINQ lessons">
              {[
                ["all", "All"],
                ["todo", "To do"],
                ["done", "Done"],
      ...
      <div className="oops-dashboard-strip">
        <div className="oops-stat-tile">
          <span>Lessons</span>
          <strong>
            {completedCount}/{CSHARP_LINQ_LESSONS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Chapters</span>
          <strong>
            {completedChapters}/{CSHARP_LINQ_CHAPTERS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>XP</span>
          <strong>
            {earnedXP}/{CSHARP_LINQ_TOTAL_XP}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Bookmarks</span>
      ...
      <CourseCertificate
        courseName="C# LINQ"
        totalLessons={CSHARP_LINQ_LESSONS.length}
        completedCount={completedCount}
        earnedXP={earnedXP}
        totalXP={CSHARP_LINQ_TOTAL_XP}
      />
    </div>
  );
}
