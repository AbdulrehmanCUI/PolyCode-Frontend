...
const READ_GATE_PREFIX = "csharp_linq";

export default function CsharpLinqLessonPage() {
  ...
  const lesson = CSHARP_LINQ_LESSONS.find((item) => item.id === lessonId);
  const lessonIdx = CSHARP_LINQ_LESSONS.findIndex(
    (item) => item.id === lessonId,
  );
  const prev = CSHARP_LINQ_LESSONS[lessonIdx - 1];
  const next = CSHARP_LINQ_LESSONS[lessonIdx + 1];
  ...
  const earnedXP = CSHARP_LINQ_LESSONS.filter((item) => progress[item.id]).reduce(
      ...
      <div className="oops-lesson-main">
        <div className="oops-lesson-topbar">
          ...
          <LearnProfileMenu
            ...
            syncLabel={
              isAuthenticated
                ? "C# LINQ progress saved to your account"
        ...
        <LessonContentShell
          tab={tab}
          storageKey={`csharp-linq:${lessonId}`}
          videoUrl={lesson.videoUrl}
          videoTitle={`${lesson.title} — C# LINQ`}
        >
          {tab === "theory" ? (
            <NumpyIntroTheory
              lesson={lesson}
