...
import {
  CSHARP_FILE_HANDLING_CHAPTERS,
  CSHARP_FILE_HANDLING_LESSONS,
  CSHARP_FILE_HANDLING_TOTAL_XP,
} from "../data/csharpFileHandlingCurriculum";
import useCsharpFileHandlingProgress from "../hooks/useCsharpFileHandlingProgress";
import LearnChapterPathOverview from "../../shared/LearnChapterPathOverview";
import LearnChapterGrid from "../../shared/LearnChapterGrid";
import CourseCertificate from "../../shared/CourseCertificate";

const BASE_PATH = "/learn/csharp-file-handling";
...
export default function CsharpFileHandlingHub() {
  ...
  const {
    ...
    bookmarks,
    lastLessonId,
  } = useCsharpFileHandlingProgress();

  const completedCount = Object.keys(progress).length;
  const earnedXP = CSHARP_FILE_HANDLING_LESSONS.filter(
    (lesson) => progress[lesson.id],
  ).reduce((sum, lesson) => sum + lesson.xp, 0);
  const pct =
    Math.round((completedCount / CSHARP_FILE_HANDLING_LESSONS.length) * 100) || 0;
  const nextLesson =
    CSHARP_FILE_HANDLING_LESSONS.find((lesson) => !progress[lesson.id]) ||
    CSHARP_FILE_HANDLING_LESSONS[0];
  const resumeLesson =
    CSHARP_FILE_HANDLING_LESSONS.find((lesson) => lesson.id === lastLessonId) ||
    nextLesson;
  const completedChapters = CSHARP_FILE_HANDLING_CHAPTERS.filter((chapter) =>
    chapter.lessons.every((lesson) => progress[lesson.id]),
  ).length;
  const bookmarkedLessons = bookmarks
    .map((id) => CSHARP_FILE_HANDLING_LESSONS.find((lesson) => lesson.id === id))
    .filter(Boolean);

  const filteredLessons = useMemo(() => {
    const query = search.trim().toLowerCase();
    return CSHARP_FILE_HANDLING_LESSONS.filter((lesson) => {
      const matchesQuery =
        !query ||
  ...
  return (
    <div className="oops-hub csharp-file-handling-hub">
      <div className="oops-hero csharp-file-handling-hero">
        <Link
          to="/select-language"
        ...
        <div className="oops-hero-badge">C# · FILE HANDLING TRACK</div>
        ...
        <p className="oops-hero-sub">
          Read and write files, manage directories, stream large files
          safely, and handle file errors with try/catch.
        </p>
        ...
        <div className="oops-hero-grid">
          <div className="oops-xp-bar-wrap">
            <div className="oops-xp-meta">
              <span>
                {isAuthenticated
                  ? `${completedCount}/${CSHARP_FILE_HANDLING_LESSONS.length} lessons · ${earnedXP}/${CSHARP_FILE_HANDLING_TOTAL_XP} XP`
                  : `Sign in to track progress · ${CSHARP_FILE_HANDLING_LESSONS.length} lessons`}
              </span>
      ...
      <div className="oops-guide-tools">
        <div className="oops-tool-panel oops-tool-panel-main">
          <span className="oops-interactive-label">Find a C# File Handling topic</span>
          <div className="oops-search-row">
            <input
              type="search"
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              placeholder="Search files, directories, streams..."
              aria-label="Search C# File Handling lessons"
            />
            <div className="oops-filter-tabs" aria-label="Filter C# File Handling lessons">
              {[
                ["all", "All"],
                ["todo", "To do"],
                ["done", "Done"],
      ...
      <div className="oops-dashboard-strip">
        <div className="oops-stat-tile">
          <span>Lessons</span>
          <strong>
            {completedCount}/{CSHARP_FILE_HANDLING_LESSONS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Chapters</span>
          <strong>
            {completedChapters}/{CSHARP_FILE_HANDLING_CHAPTERS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>XP</span>
          <strong>
            {earnedXP}/{CSHARP_FILE_HANDLING_TOTAL_XP}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Bookmarks</span>
      ...
      <LearnChapterPathOverview
        chapters={CSHARP_FILE_HANDLING_CHAPTERS}
        progress={progress}
      ...
      <CourseCertificate
        ...
        totalLessons={CSHARP_FILE_HANDLING_LESSONS.length}
        completedCount={completedCount}
        earnedXP={earnedXP}
        totalXP={CSHARP_FILE_HANDLING_TOTAL_XP}
      />
    </div>
  );
}
