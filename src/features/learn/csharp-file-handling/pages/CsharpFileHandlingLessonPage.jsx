...
const READ_GATE_PREFIX = "csharp_file_handling";

export default function CsharpFileHandlingLessonPage() {
  ...
  const lesson = CSHARP_FILE_HANDLING_LESSONS.find((item) => item.id === lessonId);
  const lessonIdx = CSHARP_FILE_HANDLING_LESSONS.findIndex(
    (item) => item.id === lessonId,
  );
  const prev = CSHARP_FILE_HANDLING_LESSONS[lessonIdx - 1];
  const next = CSHARP_FILE_HANDLING_LESSONS[lessonIdx + 1];
  ...
  const earnedXP = CSHARP_FILE_HANDLING_LESSONS.filter((item) => progress[item.id]).reduce(
  ...
  return (
    <div className={`oops-lesson-page ${focusMode ? "oops-focus-mode" : ""}`}>
      ...
      <div className="oops-lesson-main">
        ...
        <LessonContentShell
          tab={tab}
          storageKey={`csharp-file-handling:${lessonId}`}
          videoUrl={lesson.videoUrl}
          videoTitle={`${lesson.title} — C# File Handling`}
        >
          {tab === "theory" ? (
            <NumpyIntroTheory
              lesson={lesson}
