...
const READ_GATE_PREFIX = "rust-concurrency";

export default function RustConcurrencyLessonPage() {
  ...
  const lesson = RUST_CONCURRENCY_LESSONS.find((item) => item.id === lessonId);
  const lessonIdx = RUST_CONCURRENCY_LESSONS.findIndex(
    (item) => item.id === lessonId,
  );
  const prev = RUST_CONCURRENCY_LESSONS[lessonIdx - 1];
  const next = RUST_CONCURRENCY_LESSONS[lessonIdx + 1];

  useLessonAssistantContext({
    course: "Rust Concurrency",
    language: "Rust",
    lesson,
  ...
  return (
    <div className={`oops-lesson-page ${focusMode ? "oops-focus-mode" : ""}`}>
      ...
      <div className="oops-lesson-main">
        ...
        <LessonContentShell
          tab={tab}
          storageKey={`rust-concurrency:${lessonId}`}
          videoUrl={lesson.videoUrl}
          videoTitle={`${lesson.title} — Rust Concurrency`}
        >
          {tab === "theory" ? (
            <NumpyIntroTheory
              lesson={lesson}
