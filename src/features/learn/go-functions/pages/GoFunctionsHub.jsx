...
import {
  GO_FUNCTIONS_CHAPTERS,
  GO_FUNCTIONS_LESSONS,
  GO_FUNCTIONS_TOTAL_XP,
} from "../data/Go_functionsCurriculum";
import useGoFunctionsProgress from "../hooks/useGoFunctionsProgress";

const BASE_PATH = "/learn/go-functions";
const LEARNING_PATH = [
  {
    level: "Beginner",
    chapters: ["go-ch-0"],
    color: "#22c55e",
    summary: "Start with function syntax, basic calls, and return values.",
  },
  {
    level: "Intermediate",
    chapters: ["go-ch-1"],
    color: "#3b82f6",
    summary: "Handle multiple returns, errors, and defer syntax.",
  },
  {
    level: "Pro",
    chapters: ["go-ch-2", "go-ch-3"],
    color: "#f59e0b",
    summary: "Work with methods, interfaces, goroutines, and channels.",
  },
  {
    level: "Advanced",
    chapters: ["go-ch-4", "go-ch-5"],
    color: "#8b5cf6",
    summary: "Master closures, recursion, higher-order functions, and function types.",
  },
];
...
export default function GoFunctionsHub() {
  const navigate = useNavigate();
  const [search, setSearch] = useState("");
  const [filter, setFilter] = useState("all");
  const { isAuthenticated, completedMap: progress, bookmarks, lastLessonId } =
    useGoFunctionsProgress();

  const completedCount = Object.keys(progress).length;
  const earnedXP = GO_FUNCTIONS_LESSONS.filter((lesson) => progress[lesson.id]).reduce(
    (sum, lesson) => sum + lesson.xp,
    0,
  );
  const pct =
    Math.round((completedCount / GO_FUNCTIONS_LESSONS.length) * 100) || 0;
  const nextLesson =
    GO_FUNCTIONS_LESSONS.find((lesson) => !progress[lesson.id]) ||
    GO_FUNCTIONS_LESSONS[0];
  const resumeLesson =
    GO_FUNCTIONS_LESSONS.find((lesson) => lesson.id === lastLessonId) ||
    nextLesson;
  const completedChapters = GO_FUNCTIONS_CHAPTERS.filter((chapter) =>
    chapter.lessons.every((lesson) => progress[lesson.id]),
  ).length;
  const bookmarkedLessons = bookmarks
    .map((id) => GO_FUNCTIONS_LESSONS.find((lesson) => lesson.id === id))
    .filter(Boolean);

  const filteredLessons = useMemo(() => {
    const query = search.trim().toLowerCase();
    return GO_FUNCTIONS_LESSONS.filter((lesson) => {
      const matchesQuery =
        !query ||
  ...
  return (
    <div className="oops-hub go-hub">
      <div className="oops-hero go-hero" style={{ borderColor: "#00add8" }}>
        ...
        <div className="oops-hero-badge">Go · FUNCTIONS TRACK</div>
        <h1 className="oops-hero-title">
          Go
          <br />
          <span className="oops-hero-accent">Functions</span>
        </h1>
        <p className="oops-hero-sub">
          Learn Go function design from beginner syntax to concurrent goroutines,
          with guided theory, working code challenges, and real Go examples.
        </p>
        ...
        <div className="oops-hero-grid">
          <div className="oops-xp-bar-wrap">
            <div className="oops-xp-meta">
              <span>
                {isAuthenticated
                  ? `${completedCount}/${GO_FUNCTIONS_LESSONS.length} lessons · ${earnedXP}/${GO_FUNCTIONS_TOTAL_XP} XP`
                  : `Sign in to track progress · ${GO_FUNCTIONS_LESSONS.length} lessons`}
              </span>
            ...
            <div className="oops-xp-track">
              <div
                ...
                style={{ width: isAuthenticated ? `${pct}%` : "0%", backgroundColor: "#00add8" }}
              />
            </div>
          </div>
          ...
          <div className="oops-resume-panel">
            ...
            >
              {completedCount > 0 ? "Resume Go Functions" : "Start Go Functions"}
            </button>
          </div>
        </div>
      </div>
        ...
        <div className="oops-tool-panel oops-tool-panel-main">
          ...
          <div className="oops-search-row">
            <input
              ...
              placeholder="Search functions, errors, interfaces..."
              aria-label="Search Go Functions lessons"
            />
      ...
      <div className="oops-dashboard-strip">
        <div className="oops-stat-tile">
          <span>Lessons</span>
          <strong>
            {completedCount}/{GO_FUNCTIONS_LESSONS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Chapters</span>
          <strong>
            {completedChapters}/{GO_FUNCTIONS_CHAPTERS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>XP</span>
          <strong>
            {earnedXP}/{GO_FUNCTIONS_TOTAL_XP}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Bookmarks</span>
      ...
      <section className="matplotlib-learn-path" aria-label="Learning path">
        <div className="matplotlib-path-label">
          ...
          <small>
            {GO_FUNCTIONS_CHAPTERS.length} chapters · {GO_FUNCTIONS_LESSONS.length} lessons
          </small>
        </div>
        <div className="matplotlib-path-grid">
          {LEARNING_PATH.map((stage) => {
            const stageChapters = GO_FUNCTIONS_CHAPTERS.filter((ch) =>
      ...
      <LearnChapterPathOverview
        chapters={GO_FUNCTIONS_CHAPTERS}
        progress={progress}
      ...
      />
      <CourseCertificate
        courseName="Go Functions"
        totalLessons={GO_FUNCTIONS_LESSONS.length}
        completedCount={completedCount}
        earnedXP={earnedXP}
        totalXP={GO_FUNCTIONS_TOTAL_XP}
      />
    </div>
  );
}
