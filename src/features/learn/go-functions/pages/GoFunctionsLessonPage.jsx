...
const READ_GATE_PREFIX = "go_functions";

export default function GoFunctionsLessonPage() {
  ...
  const lesson = GO_FUNCTIONS_LESSONS.find((item) => item.id === lessonId);
  const lessonIdx = GO_FUNCTIONS_LESSONS.findIndex((item) => item.id === lessonId);
  const prev = GO_FUNCTIONS_LESSONS[lessonIdx - 1];
  const next = GO_FUNCTIONS_LESSONS[lessonIdx + 1];
  ...
  if (!lesson) {
    return (
      <div className="oops-not-found">
        <p>Go Functions lesson not found.</p>
        <button type="button" onClick={() => navigate(BASE_PATH)}>
          ← Back to Go Functions
        </button>
      </div>
    );
  }
  ...
  const earnedXP = GO_FUNCTIONS_LESSONS.filter((item) => progress[item.id]).reduce(
    ...
    <div className={`oops-lesson-page ${focusMode ? "oops-focus-mode" : ""}`}>
      ...
      <div className="oops-lesson-main">
        <div className="oops-lesson-topbar">
          ...
          <LearnProfileMenu
            user={user}
            trackTitle="Go Functions"
            syncLabel={
              isAuthenticated
                ? "Go Functions progress saved to your account"
        ...
        <LessonContentShell
          storageKey={`go_functions:${lessonId}`}
