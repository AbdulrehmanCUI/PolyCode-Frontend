...
export default function CsharpOopHub() {
  ...
  const earnedXP = CSHARP_OOP_LESSONS.filter(
    (lesson) => progress[lesson.id],
  ).reduce((sum, lesson) => sum + lesson.xp, 0);
  const pct =
    Math.round((completedCount / CSHARP_OOP_LESSONS.length) * 100) || 0;
  const nextLesson =
    CSHARP_OOP_LESSONS.find((lesson) => !progress[lesson.id]) ||
    CSHARP_OOP_LESSONS[0];
  const resumeLesson =
    CSHARP_OOP_LESSONS.find((lesson) => lesson.id === lastLessonId) ||
    nextLesson;
  const completedChapters = CSHARP_OOP_CHAPTERS.filter((chapter) =>
    chapter.lessons.every((lesson) => progress[lesson.id]),
  ).length;
  const bookmarkedLessons = bookmarks
    .map((id) => CSHARP_OOP_LESSONS.find((lesson) => lesson.id === id))
    .filter(Boolean);

  const filteredLessons = useMemo(() => {
    const query = search.trim().toLowerCase();
    return CSHARP_OOP_LESSONS.filter((lesson) => {
      const matchesQuery =
        !query ||
  ...
  return (
    <div className="oops-hub csharp-oop-hub">
      <div className="oops-hero csharp-oop-hero">
        <Link
          to="/select-language"
        ...
        <div className="oops-hero-badge">C# · OOP TRACK</div>
        ...
        <p className="oops-hero-sub">
          Classes, instances, and initialize; encapsulation with
          properties; inheritance, polymorphism, and interfaces in C#.
        </p>
        ...
        <div className="oops-hero-grid">
          <div className="oops-xp-bar-wrap">
            <div className="oops-xp-meta">
              <span>
                {isAuthenticated
                  ? `${completedCount}/${CSHARP_OOP_LESSONS.length} lessons · ${earnedXP}/${CSHARP_OOP_TOTAL_XP} XP`
                  : `Sign in to track progress · ${CSHARP_OOP_LESSONS.length} lessons`}
              </span>
          ...
          <div className="oops-resume-panel">
            ...
            >
              {completedCount > 0 ? "Resume C# OOP" : "Start C# OOP"}
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="oops-guide-tools">
        <div className="oops-tool-panel oops-tool-panel-main">
          <span className="oops-interactive-label">Find a C# OOP topic</span>
          <div className="oops-search-row">
            <input
              type="search"
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              placeholder="Search classes, inheritance, interfaces..."
              aria-label="Search C# OOP lessons"
            />
            <div className="oops-filter-tabs" aria-label="Filter C# OOP lessons">
              {[
                ["all", "All"],
                ["todo", "To do"],
                ["done", "Done"],
      ...
      <div className="oops-dashboard-strip">
        <div className="oops-stat-tile">
          <span>Lessons</span>
          <strong>
            {completedCount}/{CSHARP_OOP_LESSONS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Chapters</span>
          <strong>
            {completedChapters}/{CSHARP_OOP_CHAPTERS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>XP</span>
          <strong>
            {earnedXP}/{CSHARP_OOP_TOTAL_XP}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Bookmarks</span>
