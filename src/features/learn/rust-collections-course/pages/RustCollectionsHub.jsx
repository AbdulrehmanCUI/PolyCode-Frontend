...
const LEARNING_PATH = [
  {
    level: "Beginner",
    chapters: ["collections-vectors"],
    color: "#ce422b",
    summary: "Vec basics, common operations, and iterating vectors.",
  },
  {
    level: "Intermediate",
    chapters: ["collections-strings", "collections-hashmaps"],
    color: "#f59e0b",
    summary: "String vs &str, UTF-8, HashMap basics, and the entry API.",
  },
  {
    level: "Advanced",
    chapters: ["collections-sets-ordered", "collections-iterators"],
    color: "#7c3aed",
    summary: "HashSet, BTreeMap, VecDeque, and the Iterator trait in depth.",
  },
  {
    level: "Expert",
    chapters: ["collections-advanced-capstone"],
    color: "#dc2626",
    summary: "Choosing the right collection, and a word-frequency capstone project.",
  },
];
...
export default function RustCollectionsHub() {
  ...
  const pct =
    Math.round((completedCount / RUST_COLLECTIONS_LESSONS.length) * 100) || 0;

  const nextLesson =
    RUST_COLLECTIONS_LESSONS.find((lesson) => !progress[lesson.id]) ||
    RUST_COLLECTIONS_LESSONS[0];
  const resumeLesson =
    RUST_COLLECTIONS_LESSONS.find((lesson) => lesson.id === lastLessonId) ||
    nextLesson;
  const completedChapters = RUST_COLLECTIONS_CHAPTERS.filter((chapter) =>
    chapter.lessons.every((lesson) => progress[lesson.id]),
  ).length;
  const bookmarkedLessons = bookmarks
    .map((id) => RUST_COLLECTIONS_LESSONS.find((lesson) => lesson.id === id))
    .filter(Boolean);

  const filteredLessons = useMemo(() => {
    const query = search.trim().toLowerCase();
    return RUST_COLLECTIONS_LESSONS.filter((lesson) => {
      const matchesQuery =
        !query ||
  ...
  return (
    <div className="oops-hub matplotlib-hub">
      <div className="oops-hero matplotlib-hero">
        ...
        <div className="oops-hero-badge">RUST · DATA COURSE</div>
        ...
        <p className="oops-hero-sub">
          Master Rust's collection types — Vec, String, HashMap, sets,
          ordered maps, and the Iterator trait.{" "}
          {RUST_COLLECTIONS_CHAPTERS.length} chapters,{" "}
          {RUST_COLLECTIONS_LESSONS.length} lessons, hands-on Rust challenges.
        </p>
        ...
        <div className="oops-hero-grid">
          <div className="oops-xp-bar-wrap">
            <div className="oops-xp-meta">
              <span>
                {isAuthenticated
                  ? `${completedCount}/${RUST_COLLECTIONS_LESSONS.length} lessons · ${earnedXP}/${RUST_COLLECTIONS_TOTAL_XP} XP`
                  : `Sign in to track progress · ${RUST_COLLECTIONS_LESSONS.length} lessons`}
              </span>
        ...
        <div className="oops-tool-panel oops-tool-panel-main">
          ...
          <div className="oops-search-row">
            <input
              ...
              placeholder="Search Vec, HashMap, iterators..."
              aria-label="Search Rust Collections lessons"
            />
            <div
              className="oops-filter-tabs"
              aria-label="Filter Rust Collections lessons"
            >
              {[
                ["all", "All"],
                ["todo", "To do"],
      ...
      <div className="oops-dashboard-strip">
        <div className="oops-stat-tile">
          <span>Lessons</span>
          <strong>
            {completedCount}/{RUST_COLLECTIONS_LESSONS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Chapters</span>
          <strong>
            {completedChapters}/{RUST_COLLECTIONS_CHAPTERS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>XP</span>
          <strong>
            {earnedXP}/{RUST_COLLECTIONS_TOTAL_XP}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Bookmarks</span>
      ...
      <section className="matplotlib-learn-path" aria-label="Learning path">
        <div className="matplotlib-path-label">
          ...
          <small>
            {RUST_COLLECTIONS_CHAPTERS.length} chapters ·{" "}
            {RUST_COLLECTIONS_LESSONS.length} lessons
          </small>
        </div>
        <div className="matplotlib-path-grid">
          {LEARNING_PATH.map((stage) => {
            const stageChapters = RUST_COLLECTIONS_CHAPTERS.filter((ch) =>
