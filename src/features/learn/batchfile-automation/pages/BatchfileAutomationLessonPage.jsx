...
import BatchfileAutomationCodeChallenge from "../components/BatchfileAutomationCodeChallenge";
import {
...
const READ_GATE_PREFIX = "batchfile-automation";

export default function BatchfileAutomationLessonPage() {
  ...
  const lesson = BATCHFILE_AUTOMATION_LESSONS.find(
    (item) => item.id === lessonId,
  );
  const lessonIdx = BATCHFILE_AUTOMATION_LESSONS.findIndex(
    (item) => item.id === lessonId,
  );
  const prev = BATCHFILE_AUTOMATION_LESSONS[lessonIdx - 1];
  const next = BATCHFILE_AUTOMATION_LESSONS[lessonIdx + 1];

  useLessonAssistantContext({
    course: "Batchfile Automation",
    language: "Batchfile",
    lesson,
    ...
    <div className={`oops-lesson-page ${focusMode ? "oops-focus-mode" : ""}`}>
      ...
      <div className="oops-lesson-main">
        <div className="oops-lesson-topbar">
          ...
          <LearnProfileMenu
            user={user}
            trackTitle="Batchfile Automation"
            syncLabel={
              isAuthenticated
        ...
        <LessonContentShell
          tab={tab}
          storageKey={`batchfile-automation:${lessonId}`}
          videoUrl={lesson.videoUrl}
          videoTitle={`${lesson.title} — Batchfile Automation`}
        >
          {tab === "theory" ? (
            <NumpyIntroTheory
              lesson={lesson}
            ...
            <BatchfileAutomationCodeChallenge
