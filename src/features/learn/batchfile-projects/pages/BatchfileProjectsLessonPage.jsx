...
import BatchfileProjectsCodeChallenge from "../components/BatchfileProjectsCodeChallenge";
import {
...
const READ_GATE_PREFIX = "batchfile-projects";

export default function BatchfileProjectsLessonPage() {
  ...
  const lesson = BATCHFILE_PROJECTS_LESSONS.find(
    (item) => item.id === lessonId,
  );
  const lessonIdx = BATCHFILE_PROJECTS_LESSONS.findIndex(
    (item) => item.id === lessonId,
  );
  const prev = BATCHFILE_PROJECTS_LESSONS[lessonIdx - 1];
  const next = BATCHFILE_PROJECTS_LESSONS[lessonIdx + 1];

  useLessonAssistantContext({
    course: "Batchfile Projects",
    language: "Batchfile",
    lesson,
  ...
  return (
    <div className={`oops-lesson-page ${focusMode ? "oops-focus-mode" : ""}`}>
      ...
      <div className="oops-lesson-main">
        ...
        <LessonContentShell
          tab={tab}
          storageKey={`batchfile-projects:${lessonId}`}
          videoUrl={lesson.videoUrl}
          videoTitle={`${lesson.title} — Batchfile Projects`}
        >
          {tab === "theory" ? (
            <NumpyIntroTheory
              lesson={lesson}
            ...
            <BatchfileProjectsCodeChallenge
