...
import { CHAPTERS, ALL_LESSONS, TOTAL_XP } from "../data/modernCppCurriculum";
...
const LEARNING_PATH = [
  {
    ...
    chapters: ["syntax"],
    color: "#38bdf8",
    summary: "Learn modern C++ syntax: auto, range-for, and structured bindings.",
  },
  {
    level: "Intermediate",
    chapters: ["ownership", "functional"],
    color: "#f59e0b",
    summary: "Master ownership, smart pointers, lambdas, and standard algorithms.",
  },
  {
    level: "Pro",
    chapters: ["compile-time", "containers"],
    color: "#7c3aed",
    summary: "Use constexpr, consteval, optionals, variants, and string views effectively.",
  },
  {
    level: "Advanced",
    chapters: ["move", "concurrency", "design"],
    color: "#14b8a6",
    summary: "Apply move semantics, perfect forwarding, threading, and clean API design.",
  },
];
...
export default function ModernCppHub() {
  const navigate = useNavigate();
  const [search, setSearch] = useState("");
  const [filter, setFilter] = useState("all");
  const { completedMap: progress, bookmarks, lastLessonId } = useModernCppProgress();
  ...
  return (
    <div className="oops-hub modern-cpp-hub">
      <div className="oops-hero modern-cpp-hero">
        <div className="oops-hero-badge">MODERN C++ COURSE</div>
        <h1 className="oops-hero-title">
          Modern C++
          <br />
          <span className="oops-hero-accent">Beginner to Advanced</span>
        </h1>
        <p className="oops-hero-sub">
          Learn modern C++ with real working examples: auto, smart pointers, lambdas, constexpr, optionals, variants, threading, and clean API design.
        </p>
        ...
        <div className="oops-hero-grid">
          ...
          <div className="oops-resume-panel">
            ...
            <button type="button" onClick={() => navigate(`${BASE_PATH}/lesson/${resumeLesson.id}`)}>
              {completedCount > 0 ? "Resume Modern C++" : "Start Modern C++"}
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="oops-guide-tools">
        <div className="oops-tool-panel oops-tool-panel-main">
          <span className="oops-interactive-label">Find a Modern C++ lesson</span>
          <div className="oops-search-row">
            <input
              type="search"
              value={search}
              onChange={(event) => setSearch(event.target.value)}
              placeholder="Search auto, lambda, unique_ptr, constexpr..."
              aria-label="Search Modern C++ lessons"
            />
            <div className="oops-filter-tabs" aria-label="Filter Modern C++ lessons">
              {[
                ["all", "All"],
                ["todo", "To do"],
                ["done", "Done"],
          ...
          <div className="oops-search-results">
            ...
            {filteredLessons.length === 0 && (
              <p className="oops-empty-copy">No Modern C++ lessons match that search yet.</p>
            )}
          </div>
        </div>
      ...
      <section className="matplotlib-learn-path" aria-label="Learning path">
        ...
        <div className="matplotlib-path-grid">
          {LEARNING_PATH.map((stage) => {
            const stageChapters = CHAPTERS.filter((ch) => stage.chapters.includes(ch.id));
