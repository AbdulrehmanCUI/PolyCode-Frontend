...
import {
  CSHARP_ASPNET_BASICS_CHAPTERS,
  CSHARP_ASPNET_BASICS_LESSONS,
  CSHARP_ASPNET_BASICS_TOTAL_XP,
} from "../data/csharpAspnetBasicsCurriculum";
import useCsharpAspnetBasicsProgress from "../hooks/useCsharpAspnetBasicsProgress";
import LearnChapterPathOverview from "../../shared/LearnChapterPathOverview";
import LearnChapterGrid from "../../shared/LearnChapterGrid";
import CourseCertificate from "../../shared/CourseCertificate";

const BASE_PATH = "/learn/csharp-aspnet-basics";
...
export default function CsharpAspnetBasicsHub() {
  ...
  const {
    ...
    bookmarks,
    lastLessonId,
  } = useCsharpAspnetBasicsProgress();

  const completedCount = Object.keys(progress).length;
  const earnedXP = CSHARP_ASPNET_BASICS_LESSONS.filter(
    (lesson) => progress[lesson.id],
  ).reduce((sum, lesson) => sum + lesson.xp, 0);
  const pct =
    Math.round((completedCount / CSHARP_ASPNET_BASICS_LESSONS.length) * 100) || 0;
  const nextLesson =
    CSHARP_ASPNET_BASICS_LESSONS.find((lesson) => !progress[lesson.id]) ||
    CSHARP_ASPNET_BASICS_LESSONS[0];
  const resumeLesson =
    CSHARP_ASPNET_BASICS_LESSONS.find((lesson) => lesson.id === lastLessonId) ||
    nextLesson;
  const completedChapters = CSHARP_ASPNET_BASICS_CHAPTERS.filter((chapter) =>
    chapter.lessons.every((lesson) => progress[lesson.id]),
  ).length;
  const bookmarkedLessons = bookmarks
    .map((id) => CSHARP_ASPNET_BASICS_LESSONS.find((lesson) => lesson.id === id))
    .filter(Boolean);

  const filteredLessons = useMemo(() => {
    const query = search.trim().toLowerCase();
    return CSHARP_ASPNET_BASICS_LESSONS.filter((lesson) => {
      const matchesQuery =
        !query ||
  ...
  return (
    <div className="oops-hub csharp-aspnet-basics-hub">
      <div className="oops-hero csharp-aspnet-basics-hero">
        <Link
          to="/select-language"
        ...
        <div className="oops-hero-badge">C# · ASP.NET TRACK</div>
        <h1 className="oops-hero-title">
          C#
          <br />
          <span className="oops-hero-accent">ASP.NET Basics</span>
        </h1>
        <p className="oops-hero-sub">
          Minimal APIs, routing, DTOs, dependency injection, and the
          middleware pipeline — the core patterns behind ASP.NET Core.
        </p>
        ...
        <div className="oops-hero-grid">
          <div className="oops-xp-bar-wrap">
            <div className="oops-xp-meta">
              <span>
                {isAuthenticated
                  ? `${completedCount}/${CSHARP_ASPNET_BASICS_LESSONS.length} lessons · ${earnedXP}/${CSHARP_ASPNET_BASICS_TOTAL_XP} XP`
                  : `Sign in to track progress · ${CSHARP_ASPNET_BASICS_LESSONS.length} lessons`}
              </span>
          ...
          <div className="oops-resume-panel">
            ...
            >
              {completedCount > 0 ? "Resume C# ASP.NET Basics" : "Start C# ASP.NET Basics"}
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="oops-guide-tools">
        <div className="oops-tool-panel oops-tool-panel-main">
          <span className="oops-interactive-label">Find an ASP.NET Basics topic</span>
          <div className="oops-search-row">
            <input
              type="search"
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              placeholder="Search routing, DI, middleware..."
              aria-label="Search C# ASP.NET Basics lessons"
            />
            <div className="oops-filter-tabs" aria-label="Filter C# ASP.NET Basics lessons">
              {[
                ["all", "All"],
                ["todo", "To do"],
                ["done", "Done"],
      ...
      <div className="oops-dashboard-strip">
        <div className="oops-stat-tile">
          <span>Lessons</span>
          <strong>
            {completedCount}/{CSHARP_ASPNET_BASICS_LESSONS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Chapters</span>
          <strong>
            {completedChapters}/{CSHARP_ASPNET_BASICS_CHAPTERS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>XP</span>
          <strong>
            {earnedXP}/{CSHARP_ASPNET_BASICS_TOTAL_XP}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Bookmarks</span>
      ...
      <LearnChapterPathOverview
        chapters={CSHARP_ASPNET_BASICS_CHAPTERS}
        progress={progress}
      ...
      />
      <CourseCertificate
        courseName="C# ASP.NET Basics"
        totalLessons={CSHARP_ASPNET_BASICS_LESSONS.length}
        completedCount={completedCount}
        earnedXP={earnedXP}
        totalXP={CSHARP_ASPNET_BASICS_TOTAL_XP}
      />
    </div>
  );
}
