...
const READ_GATE_PREFIX = "csharp_aspnet_basics";

export default function CsharpAspnetBasicsLessonPage() {
  ...
  const lesson = CSHARP_ASPNET_BASICS_LESSONS.find((item) => item.id === lessonId);
  const lessonIdx = CSHARP_ASPNET_BASICS_LESSONS.findIndex(
    (item) => item.id === lessonId,
  );
  const prev = CSHARP_ASPNET_BASICS_LESSONS[lessonIdx - 1];
  const next = CSHARP_ASPNET_BASICS_LESSONS[lessonIdx + 1];

  useLessonAssistantContext({
    course: "C# ASP.NET Basics",
    language: "C#",
    lesson,
  ...
  if (!lesson) {
    return (
      <div className="oops-not-found">
        <p>C# ASP.NET Basics lesson not found.</p>
        <button type="button" onClick={() => navigate(BASE_PATH)}>
          ← Back to C# ASP.NET Basics
        </button>
      </div>
    );
  }
  ...
  const earnedXP = CSHARP_ASPNET_BASICS_LESSONS.filter((item) => progress[item.id]).reduce(
    ...
    <div className={`oops-lesson-page ${focusMode ? "oops-focus-mode" : ""}`}>
      ...
      <div className="oops-lesson-main">
        <div className="oops-lesson-topbar">
          ...
          <LearnProfileMenu
            user={user}
            trackTitle="C# ASP.NET Basics"
            syncLabel={
              isAuthenticated
                ? "C# ASP.NET Basics progress saved to your account"
        ...
        <LessonContentShell
          tab={tab}
          storageKey={`csharp-aspnet-basics:${lessonId}`}
          videoUrl={lesson.videoUrl}
          videoTitle={`${lesson.title} — C# ASP.NET Basics`}
        >
          {tab === "theory" ? (
            <NumpyIntroTheory
              lesson={lesson}
