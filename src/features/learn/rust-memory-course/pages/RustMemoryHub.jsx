...
const LEARNING_PATH = [
  {
    level: "Beginner",
    chapters: ["memory-stack-heap"],
    color: "#ce422b",
    summary: "Stack vs heap, Box<T>, and the Drop trait.",
  },
  {
    level: "Intermediate",
    chapters: ["memory-ownership-deep-dive", "memory-borrowing-lifetimes-intro"],
    color: "#f59e0b",
    summary: "Move semantics, Copy vs Clone, and why the borrow checker exists.",
  },
  {
    level: "Advanced",
    chapters: ["memory-lifetimes-practice", "memory-smart-pointers"],
    color: "#7c3aed",
    summary: "Lifetime annotations, elision, Rc, RefCell, and Weak.",
  },
  {
    level: "Expert",
    chapters: ["memory-advanced-capstone"],
    color: "#dc2626",
    summary: "Unsafe Rust, raw pointers, and a boxed linked-list capstone.",
  },
];
...
export default function RustMemoryHub() {
  ...
  const pct =
    Math.round((completedCount / RUST_MEMORY_LESSONS.length) * 100) || 0;

  const nextLesson =
    RUST_MEMORY_LESSONS.find((lesson) => !progress[lesson.id]) ||
    RUST_MEMORY_LESSONS[0];
  const resumeLesson =
    RUST_MEMORY_LESSONS.find((lesson) => lesson.id === lastLessonId) ||
    nextLesson;
  const completedChapters = RUST_MEMORY_CHAPTERS.filter((chapter) =>
    chapter.lessons.every((lesson) => progress[lesson.id]),
  ).length;
  const bookmarkedLessons = bookmarks
    .map((id) => RUST_MEMORY_LESSONS.find((lesson) => lesson.id === id))
    .filter(Boolean);

  const filteredLessons = useMemo(() => {
    const query = search.trim().toLowerCase();
    return RUST_MEMORY_LESSONS.filter((lesson) => {
      const matchesQuery =
        !query ||
  ...
  return (
    <div className="oops-hub matplotlib-hub">
      <div className="oops-hero matplotlib-hero">
        ...
        <div className="oops-hero-badge">RUST · MEMORY COURSE</div>
        ...
        <p className="oops-hero-sub">
          Go deep on memory — stack vs heap, ownership internals, lifetimes,
          smart pointers, and unsafe Rust. {RUST_MEMORY_CHAPTERS.length}{" "}
          chapters, {RUST_MEMORY_LESSONS.length} lessons, hands-on Rust
          challenges.
        </p>
        ...
        <div className="oops-hero-grid">
          <div className="oops-xp-bar-wrap">
            <div className="oops-xp-meta">
              <span>
                {isAuthenticated
                  ? `${completedCount}/${RUST_MEMORY_LESSONS.length} lessons · ${earnedXP}/${RUST_MEMORY_TOTAL_XP} XP`
                  : `Sign in to track progress · ${RUST_MEMORY_LESSONS.length} lessons`}
              </span>
        ...
        <div className="oops-tool-panel oops-tool-panel-main">
          ...
          <div className="oops-search-row">
            <input
              ...
              placeholder="Search lifetimes, Rc, unsafe..."
              aria-label="Search Rust Memory lessons"
            />
            <div
              className="oops-filter-tabs"
              aria-label="Filter Rust Memory lessons"
            >
              {[
                ["all", "All"],
                ["todo", "To do"],
      ...
      <div className="oops-dashboard-strip">
        <div className="oops-stat-tile">
          <span>Lessons</span>
          <strong>
            {completedCount}/{RUST_MEMORY_LESSONS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Chapters</span>
          <strong>
            {completedChapters}/{RUST_MEMORY_CHAPTERS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>XP</span>
          <strong>
            {earnedXP}/{RUST_MEMORY_TOTAL_XP}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Bookmarks</span>
      ...
      <section className="matplotlib-learn-path" aria-label="Learning path">
        <div className="matplotlib-path-label">
          ...
          <small>
            {RUST_MEMORY_CHAPTERS.length} chapters ·{" "}
            {RUST_MEMORY_LESSONS.length} lessons
          </small>
        </div>
        <div className="matplotlib-path-grid">
          {LEARNING_PATH.map((stage) => {
            const stageChapters = RUST_MEMORY_CHAPTERS.filter((ch) =>
