...
const LEARNING_PATH = [
  {
    level: "Beginner",
    chapters: ["animations-transitions-basics"],
    color: "#8b5cf6",
    summary: "transition-property, duration, timing functions, and the transition shorthand.",
  },
  {
    level: "Intermediate",
    chapters: ["animations-transforms", "animations-keyframes"],
    color: "#f59e0b",
    summary: "translate/scale/rotate, transform-origin, and full @keyframes animations.",
  },
  {
    level: "Advanced",
    chapters: ["animations-advanced-techniques", "animations-performance-accessibility"],
    color: "#7c3aed",
    summary: "Multi-step keyframes, staggering, GPU-friendly properties, and prefers-reduced-motion.",
  },
  {
    level: "Expert",
    chapters: ["animations-practical-capstone"],
    color: "#dc2626",
    summary: "Loading spinners, micro-interactions, and a capstone animated 3D card flip.",
  },
];
...
export default function CssAnimationsHub() {
  ...
  const pct =
    Math.round((completedCount / CSS_ANIMATIONS_LESSONS.length) * 100) || 0;

  const nextLesson =
    CSS_ANIMATIONS_LESSONS.find((lesson) => !progress[lesson.id]) ||
    CSS_ANIMATIONS_LESSONS[0];
  const resumeLesson =
    CSS_ANIMATIONS_LESSONS.find((lesson) => lesson.id === lastLessonId) ||
    nextLesson;
  const completedChapters = CSS_ANIMATIONS_CHAPTERS.filter((chapter) =>
    chapter.lessons.every((lesson) => progress[lesson.id]),
  ).length;
  const bookmarkedLessons = bookmarks
    .map((id) => CSS_ANIMATIONS_LESSONS.find((lesson) => lesson.id === id))
    .filter(Boolean);

  const filteredLessons = useMemo(() => {
    const query = search.trim().toLowerCase();
    return CSS_ANIMATIONS_LESSONS.filter((lesson) => {
      const matchesQuery =
        !query ||
    ...
    <div className="oops-hub matplotlib-hub">
      <div className="oops-hero matplotlib-hero">
        ...
        <p className="oops-hero-sub">
          Bring interfaces to life — transitions, transforms, @keyframes animations, and performance-conscious, accessible motion design. {CSS_ANIMATIONS_CHAPTERS.length}{" "}
          chapters, {CSS_ANIMATIONS_LESSONS.length} lessons, hands-on challenges.
        </p>
        ...
        <div className="oops-hero-grid">
          <div className="oops-xp-bar-wrap">
            <div className="oops-xp-meta">
              <span>
                {isAuthenticated
                  ? `${completedCount}/${CSS_ANIMATIONS_LESSONS.length} lessons · ${earnedXP}/${CSS_ANIMATIONS_TOTAL_XP} XP`
                  : `Sign in to track progress · ${CSS_ANIMATIONS_LESSONS.length} lessons`}
              </span>
      ...
      <div className="oops-guide-tools">
        <div className="oops-tool-panel oops-tool-panel-main">
          <span className="oops-interactive-label">Find a CSS Animations & Transitions topic</span>
          <div className="oops-search-row">
            <input
              type="search"
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              placeholder="Search transitions, keyframes, transforms..."
              aria-label="Search CSS Animations & Transitions lessons"
            />
            <div
              className="oops-filter-tabs"
              aria-label="Filter CSS Animations & Transitions lessons"
            >
              {[
                ["all", "All"],
                ["todo", "To do"],
      ...
      <div className="oops-dashboard-strip">
        <div className="oops-stat-tile">
          <span>Lessons</span>
          <strong>
            {completedCount}/{CSS_ANIMATIONS_LESSONS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Chapters</span>
          <strong>
            {completedChapters}/{CSS_ANIMATIONS_CHAPTERS.length}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>XP</span>
          <strong>
            {earnedXP}/{CSS_ANIMATIONS_TOTAL_XP}
          </strong>
        </div>
        <div className="oops-stat-tile">
          <span>Bookmarks</span>
      ...
      <section className="matplotlib-learn-path" aria-label="Learning path">
        <div className="matplotlib-path-label">
          ...
          <small>
            {CSS_ANIMATIONS_CHAPTERS.length} chapters ·{" "}
            {CSS_ANIMATIONS_LESSONS.length} lessons
          </small>
        </div>
        <div className="matplotlib-path-grid">
          {LEARNING_PATH.map((stage) => {
            const stageChapters = CSS_ANIMATIONS_CHAPTERS.filter((ch) =>
      ...
      <CourseCertificate
        courseName="CSS Animations & Transitions"
