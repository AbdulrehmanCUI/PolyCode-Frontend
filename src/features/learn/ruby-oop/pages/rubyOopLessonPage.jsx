...
export default function RubyOopLessonPage() {
  ...
  const { user, isAuthenticated, completedMap: progress, savedCodeMap, bookmarks, completeLesson, rememberLesson, saveCode, toggleBookmark } = useRubyOopProgress();
  const codeSaveTimer = useRef(null);

  const lesson = RUBY_OOP_LESSONS.find((item) => item.id === lessonId);
  const lessonIdx = RUBY_OOP_LESSONS.findIndex((item) => item.id === lessonId);
  ...
  if (!lesson) {
    return (
      <div className="oops-not-found">
        ...
        <button type="button" onClick={() => navigate(BASE_PATH)}>← Back to Ruby OOP</button>
      </div>
    );
  }
  ...
  const earnedXP = RUBY_OOP_LESSONS.filter((item) => progress[item.id]).reduce((sum, item) => sum + item.xp, 0);
  ...
  return (
    <div className={`oops-lesson-page ${focusMode ? "oops-focus-mode" : ""}`}>
      <OopsSidebar currentLessonId={lessonId} progress={progress} chapters={RUBY_OOP_CHAPTERS} basePath={BASE_PATH} title="Ruby OOP" />

      <div className="oops-lesson-main">
        <div className="oops-lesson-topbar">
          <button type="button" className="oops-back-btn" onClick={() => navigate(BASE_PATH)}>← Ruby OOP</button>
          ...
          <LearnProfileMenu user={user} trackTitle="Ruby OOP" syncLabel={isAuthenticated ? "Ruby OOP progress saved to your account" : "Sign in to save progress"} completedCount={completedCount} totalLessons={RUBY_OOP_LESSONS.length} earnedXP={earnedXP} totalXP={RUBY_OOP_TOTAL_XP} bookmarksCount={bookmarks.length} streak={0} />
        </div>
        ...
        <LessonContentShell tab={tab} storageKey={`ruby-oop:${lessonId}`} videoUrl={lesson.videoUrl} videoTitle={`${lesson.title} — Ruby OOP`}>
          {tab === "theory" ? (
            <NumpyIntroTheory
              lesson={lesson}
