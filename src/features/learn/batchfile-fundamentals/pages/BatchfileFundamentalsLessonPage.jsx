...
import BatchfileFundamentalsCodeChallenge from "../components/BatchfileFundamentalsCodeChallenge";
import {
...
const READ_GATE_PREFIX = "batchfile-fundamentals";

export default function BatchfileFundamentalsLessonPage() {
  ...
  const lesson = BATCHFILE_FUNDAMENTALS_LESSONS.find(
    (item) => item.id === lessonId,
  );
  const lessonIdx = BATCHFILE_FUNDAMENTALS_LESSONS.findIndex(
    (item) => item.id === lessonId,
  );
  const prev = BATCHFILE_FUNDAMENTALS_LESSONS[lessonIdx - 1];
  const next = BATCHFILE_FUNDAMENTALS_LESSONS[lessonIdx + 1];

  useLessonAssistantContext({
    course: "Batchfile Fundamentals",
    language: "Batchfile",
    lesson,
    ...
    return (
      <div className="oops-not-found">
        ...
        <button type="button" onClick={() => navigate(BASE_PATH)}>
          ← Back to Batchfile Fundamentals
        </button>
      </div>
    );
  }
    ...
    <div className={`oops-lesson-page ${focusMode ? "oops-focus-mode" : ""}`}>
      ...
      <div className="oops-lesson-main">
        <div className="oops-lesson-topbar">
          ...
          <LearnProfileMenu
            user={user}
            trackTitle="Batchfile Fundamentals"
            syncLabel={
              isAuthenticated
        ...
        <LessonContentShell
          tab={tab}
          storageKey={`batchfile-fundamentals:${lessonId}`}
          videoUrl={lesson.videoUrl}
          videoTitle={`${lesson.title} — Batchfile Fundamentals`}
        >
          {tab === "theory" ? (
            <NumpyIntroTheory
              lesson={lesson}
            ...
            <BatchfileFundamentalsCodeChallenge
